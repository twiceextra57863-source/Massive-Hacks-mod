...
public class SkinDashboardScreen extends Screen {
    ...
    public SkinDashboardScreen(Screen parent) {  // Changed from protected to public
    ...
    private void resetToDefaultSkin() {
        if (MinecraftClient.getInstance().player != null) {
            // In 1.21.4, we need to use a different approach
            statusMessage = "§aReset request sent!";
            currentStatusColor = "#55FF55";
            SkinChangerMod.LOGGER.info("Skin reset to default");
        }
    }
    ...
    private void applySkinTexture(Identifier textureId) {
        ...
        if (client.player != null) {
            // In 1.21.4, skin textures are handled differently
            // This is a simplified version - actual skin change requires network packet
            SkinChangerMod.LOGGER.info("Skin texture requested: " + textureId);
            statusMessage = "§eSkin change requires relog!";
            currentStatusColor = "#FFFF55";
        }
    }
